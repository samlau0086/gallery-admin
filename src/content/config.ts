...
const products = defineCollection({ type: 'content', schema: z.object({
  title: localizedString, category: localizedString, brand:z.string().optional(), sku:z.string().optional(), cover:z.string().min(1),
  media:z.array(z.object({url:z.string().min(1),type:z.enum(['image','video']).default('image'),alt:z.string().optional()})).default([]),
  price:z.string().optional(), description: localizedString.optional(), tags:z.array(z.string()).default([]),
