...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const path = `src/content/reviews/${slug}.md`; const frontmatter = [`product: ${JSON.stringify(product)}`,`author: ${JSON.stringify(author)}`,`email: ${JSON.stringify(email)}`,`rating: ${rating}`,`title: ${JSON.stringify(String(payload.title || ''))}`,`body: ${JSON.stringify(body)}`,`date: ${JSON.stringify(new Date().toISOString().slice(0,10))}`,`variants: ${JSON.stringify(String(payload.variants || ''))}`,`status: "pending"`].join('\n'); const content = `---\n${frontmatter}\n---\n`;
