...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const path = `src/content/reviews/${slug}.md`; const frontmatter = [`product: "${product}"`,`author: "${author.replace(/"/g, '\\"')}"`,`email: "${email}"`,`rating: ${rating}`,`title: "${String(payload.title || '').replace(/"/g, '\\"')}"`,`date: "${new Date().toISOString().slice(0,10)}"`,`variants: "${String(payload.variants || '').replace(/"/g, '\\"')}"`,`status: "pending"`].join('\n'); const content = `---\n${frontmatter}\n---\n\n${body.replace(/\r?\n/g, '\n')}\n`;
