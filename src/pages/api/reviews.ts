...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const product = String(payload.product || '').trim(); const author = String(payload.name || '').trim(); const email = String(payload.email || '').trim(); const body = String(payload.body || '').trim(); const rating = Number(payload.rating); const images = Array.isArray(payload.images) ? payload.images.filter((item): item is string => typeof item === 'string').slice(0, 6) : [];
  ...
  const path = `src/content/reviews-pending/${slug}.md`; const frontmatter = [`product: ${JSON.stringify(product)}`,`author: ${JSON.stringify(author)}`,`email: ${JSON.stringify(email)}`,`rating: ${rating}`,`title: ${JSON.stringify(String(payload.title || ''))}`,`review: ${JSON.stringify(body)}`,`images: ${JSON.stringify(images)}`,`date: ${JSON.stringify(new Date().toISOString().slice(0,10))}`,`variants: ${JSON.stringify(String(payload.variants || ''))}`,`status: "pending"`].join('\n'); const content = `---\n${frontmatter}\n---\n`;
