...
for (const [index, record] of records.entries()) {
  ...
  const title = String(record['标题'] ?? '').trim();
  const images = Array.isArray(record['图片']) ? record['图片'].filter(Boolean) : [];

  if (!sku || !title || images.length === 0) {
    console.warn(`Skipping record ${index + 1}: SKU, title, and at least one image are required.`);
    skipped += 1;
    continue;
  }

  const filename = `${brand.toLowerCase()}-${safeFilename(sku)}.md`;
