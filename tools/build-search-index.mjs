...
for (const file of files) {
  ...
  records.push({
    slug, title, titleZh, category, brand, sku, cover, sortOrder, searchable,
...
await writeFile(searchOutputPath, JSON.stringify(records.map(({ slug, title, titleZh, category, brand, sku, cover, sortOrder, searchable }) => ({ slug, title, titleZh, category, brand, sku, cover, sortOrder, searchable }))), 'utf8');
await mkdir(productsOutputDir, { recursive: true });
await Promise.all(records.map((record) => writeFile(path.join(productsOutputDir, `${record.slug}.json`), JSON.stringify(record), 'utf8')));
